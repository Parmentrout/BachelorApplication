"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
require("rxjs/Rx");
var Contestantcomponent = require("../models/contestant.component");
var ContestantsService = (function () {
    function ContestantsService(_http) {
        this._http = _http;
    }
    ContestantsService.prototype.getContestants = function () {
        return this._http.get('/api/SampleData/GetContestants').map(function (result) { return result.json(); });
    };
    ContestantsService.prototype.saveContestants = function (contestants) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json; charset=utf-8' });
        var options = new http_1.RequestOptions({ headers: headers });
        this._http.post('/api/SampleData/PostChanges', contestants, options)
            .subscribe(function (result) {
            alert(result.json());
        });
    };
    ContestantsService.prototype.saveFantasy = function (user, contestant) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json; charset=utf-8' });
        var options = new http_1.RequestOptions({ headers: headers });
        var data = new Contestantcomponent.Contestant();
        data.name = contestant;
        data.userName = user;
        this._http.post('/api/SampleData/SaveFantasy', data, options)
            .subscribe(function (result) {
            alert(result.json());
        });
    };
    return ContestantsService;
}());
ContestantsService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http])
], ContestantsService);
exports.ContestantsService = ContestantsService;
//# sourceMappingURL=contestants.service.js.map